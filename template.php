 ...
 *   mission_respondable_. For example:
 *
 *     original, found in modules/field/field.module: theme_field()
 *     theme override, found in template.php: mission_respondable_field()
 *
 *   where mission_respondable is the name of your sub-theme. For example, the
...
function mission_respondable_preprocess_node(&$variables, $hook) {
  ...
  // mission_respondable_preprocess_node_page() or mission_respondable_preprocess_node_story().
...
function mission_respondable_form_alter(&$form, &$form_state, $form_id) {
  if ($form_id == 'views_exposed_form' && $form['#id'] == 'views-exposed-form-search-panel-pane-1') {
      $form['#title'] = t('Searchz'); // Change the text on the label element
      $form['#title_display'] = 'invisible'; // Toggle label visibilty
      $form['#size'] = 40;  // define size of the textfield
      $form['#default_value'] = t('Search'); // Set a default value for the textfield
      $form['submit']['#value'] = t('GO!'); // Change the text on the submit button
      $form['submit'] = array('#type' => 'image_button', '#src' => base_path() . path_to_theme() . '/images/search-button.png');

      // Add extra attributes to the text box
      $form['#attributes']['onblur'] = "if (this.value == '') {this.value = 'Search';}";
      $form['#attributes']['onfocus'] = "if (this.value == 'Search') {this.value = '';}";
      // Prevent user from searching the default text
      $form['#attributes']['onsubmit'] = "if(this.views_exposed_form_search_panel_pane_1.value=='Search'){ alert('Please enter a search'); return false; }";

      // Alternative (HTML5) placeholder attribute instead of using the javascript
      $form['#attributes']['placeholder'] = t('Search');
  }
} 
